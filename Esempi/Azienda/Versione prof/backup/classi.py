...
class Progetto:
    _nome: str # noto alla nascita
    _budget: Importo
    _impiegati_prog: _imp_progetto  # mutabile e non noti alla nascita

    def __init__(self, nome:str, budget:Importo) -> None:
        self.set_nome(nome)
        self.set_budget(budget)
        self._impiegati_prog = set()
    ...
    def add_impiegati_prog(self, ip: _imp_progetto) -> None:
        self._impiegati_prog.add(ip)
    
    def remove_impiegati_prog(self, ip: _imp_progetto) -> None:
        if len(self._impiegati_prog) >= 1 and ip in self._impiegati_prog:
            self._impiegati_prog.remove(ip)
        
        else:
            raise ValueError(f"Errore, non c'è questo impiegato nel progetto '{self._nome}'")

    def budget(self) -> Importo:
        return self._budget 
    
    def getImpiegatiProg(self) -> frozenset[_imp_progetto]:
        return frozenset(self._impiegati_prog)
...
class _imp_progetto:
    class _link:
        ...
        def getDataAfferenzaProgetto(self) -> date:
            return self._data_afferenza_prog
        
        def __hash__(self) -> int:
            return hash((self.getImpiegato(), self.getProgetto()))
        
        def __eq__(self, other) -> bool:
            if type(self) != type(other) or hash(self) != hash(other):
                return False
            return(self.getImpiegato(), self.getProgetto()) == (other.getImpiegato(), other.getProgetto())
