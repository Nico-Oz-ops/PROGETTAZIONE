...
class Impiegato:
    ...
    _progetti: dict[Progetto, _imp_prog]
    ...
    def progetti(self) -> frozenset[_imp_prog]:
    ...
    def add_progetto(self, progetto: Progetto, data: date) -> None:
        if progetto in self._progetti:
            raise KeyError(f"Errore, il progetto esiste già")
    ...
    def data_coinvolgimento(self, progetto: Progetto) -> None:
        try:
            self._progetti[progetto].data()
